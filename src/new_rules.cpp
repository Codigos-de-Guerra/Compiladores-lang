...
decl_fun::decl_fun(type_name *t, typedlpar *tlpar, block *bl) {
  ...
  if (bl != NULL)
    intermid += bl->intermid;
}

statement::statement(decl_fun *decl_f) {
  if (decl_f != NULL)
    intermid = decl_f->intermid;
}
...
identifier::identifier(string nome) { name = nome; }
...
expr::expr(state &estado, expr *left, string operacao, expr *right) {
  ...
  ret += left->ret;
  ret += right->ret;
  ret += intermid + " = " + left->intermid + " " + operacao + " " +
         right->intermid + ";\n";
  ...
  if (left->type != right->type) {
    if (compatibilidade.find(make_pair(left->type, right->type)) ==
        compatibilidade.end()) {
...
expr::expr(state &estado, string operacao, expr *exp) {
  ...
  if (operacao == "++" or operacao == "--") {
    if (exp->isId) {
      ...
      // TODO
      //  Tentativa de ter algo como ++(++a) recursivo que de fato incremente
      //  "a" multiplas vezes
    }
...
all_decl_var::all_decl_var(state &estado, decl_var_prim *var) {
  ...
  if (sym != local_table.end()) {
    cout << var->name << " já foi declarado como " << sym->second.first << endl;
    estado.deuErro = true;
  } else
...
cmd_decl_var::cmd_decl_var(state &estado, all_decl_var *a,
                           assign_expr_maybe *b) {
  if (b == NULL)
    return;
  if (!b->intermid.empty()) {
...
caze::caze(literal *t, statements *stmts, bool hasBreak) {
  tipo = t->type;
  valor = t->value;
  intermid = stmts->intermid;
...
cazezeromais::cazezeromais(state &estado, caze *c, cazezeromais *cs) {
  if (cs->valorEsperado != "")
    valorEsperado = cs->valorEsperado;
  else
    valorEsperado = c->tipo;

  javi = cs->javi;

  if (c->tipo != valorEsperado) {
    if (compatibilidade.find(make_pair(c->tipo, valorEsperado)) ==
        compatibilidade.end()) {
      cout << c->tipo << " " << valorEsperado << endl;

      cout << "Tipo incompativel no switch" << endl;
      estado.deuErro = true;
    }
  } else if (javi.find(c->valor) != javi.end()) {
    cout << "Case já inserido" << endl;
    estado.deuErro = true;
  } else
    javi.insert(c->valor);

  cazeIntermid = cs->cazeIntermid;
  cazeIntermid.push_back({c->valor, c->intermid,
                          "l" + to_string(estado.labelId++),
                          c->hasBreak ? "break" : ""});
}
...
cmd::cmd(cmd_switch *c_sw) {
  if (c_sw != NULL)
    ret = c_sw->intermid;
}

cmd::cmd(state &estado, expr *exp) { ret = exp->ret; }
...
cmd::cmd(state &estado, string s) {
  if (s == "CONTINUE")
    ret = "goto l" + to_string(estado.labelId) + ";\n";
}
...
cmd_cond::cmd_cond(ifa *iff) {
  if (iff != NULL)
    intermid = iff->intermid;
}

cmd_loop::cmd_loop(loop *lo) {
  if (lo != NULL)
    intermid = lo->intermid;
}
...
switcha::switcha(state &estado, expr *exp, cazezeromais *cases) {
  if (cases->valorEsperado == "")
    return;

  if (exp->type != cases->valorEsperado) {
    if (compatibilidade.find(make_pair(exp->type, cases->valorEsperado)) ==
        compatibilidade.end()) {
      cout << exp->type << " " << cases->valorEsperado << endl;
      cout << "Tipo incompativel no switch" << endl;
      estado.deuErro = true;
    }
  }
  ...
  // intermid = exp->intermid;
  string labelOutSwitch = "l" + to_string(estado.labelId++);
  vector<vector<string>>::iterator i;

  for (i = cases->cazeIntermid.begin(); i != cases->cazeIntermid.end(); i++) {
    // cout << "if (" << exp->intermid
    string t1 = "t" + to_string(estado.nxtId++);
    string t2 = "t" + to_string(estado.nxtId++);
    string t1_e = t1 + " = " + i->at(0) + ";";
    string t2_e = t2 + " = " + exp->intermid + " == " + t1 + ";";
    string labelTrue = "l" + to_string(estado.labelId++);

    intermid += i->at(2) + ":" + "\n";
    intermid += t1_e + "\n";
    intermid += t2_e + "\n";
    intermid += "if (" + t2 + ")" + " goto " + labelTrue + ";" + "\n";
    if (i + 1 != cases->cazeIntermid.end()) {
      intermid += "goto " + (i + 1)->at(2) + ";" + "\n";
    } else {
      intermid += "goto " + labelOutSwitch + ";" + "\n";
    }
    intermid += labelTrue + ":" + "\n" + i->at(1);
    if (i->at(3) != "") {
      intermid += "goto " + labelOutSwitch + ";" + "\n";
  ...
  intermid += labelOutSwitch + ":" + "\n";
}

cmd_switch::cmd_switch(switcha *sw) {
  if (sw != NULL) {
    intermid = sw->intermid;
  }
}
